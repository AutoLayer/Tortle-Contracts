    ...
    function batchFunctions(Function[] memory _functions) public onlyOwner {
        for (uint256 i = 0; i < _functions.length; i++) {
            ...
            if (StringUtils.equal(_functionName, 'addFundsForTokens')) {
                ...
                uint256 amount = nodes.addFundsForTokens(_user, IERC20(_token), _amount);
                if (_hasNext) {
                ...
                _splitStruct.percentageFirstToken = StringUtils.safeParseInt(_arguments[4]);
                _splitStruct.amountOutMinFirst = StringUtils.safeParseInt(_arguments[5]);
                _splitStruct.amountOutMinSecond = StringUtils.safeParseInt(_arguments[6]);
                ...
                if (auxStack.length > 0) {
                    ...
                    _splitStruct.amount = StringUtils.safeParseInt(_arguments[1]);
                }
                ...
                emit Split(_id, _splitStruct.token, _splitStruct.amount, amountOutToken1, amountOutToken2);
                ...
                uint256 amountOut = nodes.swapTokens(_user, IERC20(_token), _amount, _newToken, _amountOutMin);
                if (_hasNext) {
                ...
                uint256 y;
                for (uint256 x = _tokenArguments; x < _arguments.length - 1; x++) {
                    uint256 _amount;
                    if (auxStack.length > 0) {
                        _amount = auxStack[auxStack.length - 1];
                        auxStack.pop();
                ...
                address _tokenOutput = StringUtils.parseAddr(_arguments[_arguments.length - 1]);

                uint256 amountOut = nodes.liquidate(_user, _tokens, _amounts, _tokenOutput);
